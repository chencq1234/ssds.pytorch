...
from lib.calpr.prlib.BoundingBox import *
from lib.calpr.prlib.BoundingBoxes import *
from lib.calpr.prlib.utils import *
...
class Evaluator:
    def GetPascalVOCMetrics(self,
        ...
        Args:
            ...
            bounding boxes;
            IOUThreshold: IOU threshold indicating which pr_detections will be considered TP or FP
            (default value = 0.5);
        ...
        Returns:
            ...
            dict['total TP']: total number of True Positive pr_detections;
            dict['total FP']: total number of False Negative pr_detections;
        """
        ret = []  # list containing metrics (precision, recall, average precision) of each class
        # List with all ground truths (Ex: [imageName,class,confidence=1, (bb coordinates XYX2Y2)])
        groundTruths = []
        # List with all pr_detections (Ex: [imageName,class,confidence,(bb coordinates XYX2Y2)])
        detections = []
        # Get all classes
        classes = []
        # Loop through all bounding boxes and separate them into GTs and pr_detections
        ...
        for c in classes:
            ...
            npos = len(gts)
            # sort pr_detections by decreasing confidence
            ...
            # print("Evaluating class: %s (%d pr_detections)" % (str(c), len(dects)))
            # Loop through pr_detections
            for d in range(len(dects)):
                # print('dect %s => %s' % (dects[d][0], dects[d][3],))
                # Find ground truth image 找到dects[d]对应的gt
            ...
            # add class results in the dictionary to be returned
            r = {
                'class': c,
                'precision': prec,
                'recall': rec,
    ...
    def PlotPrecisionRecallCurve(self,
        ...
        Args:
            boundingBoxes: Object of the class BoundingBoxes representing ground truth and detected
            bounding boxes;
            IOUThreshold (optional): IOU threshold indicating which pr_detections will be considered
        ...
        # Each resut represents a class
            for result in results:
                ...
                plt.plot(recall, precision, label=str(classId))
                plt.xlabel('recall')
                ...
                if showAP:
                    ...
                    # plt.title('Precision x Recall curve \nClass: %s' % classId)
                    print('Precision x Recall curve \nClass: %s' % classId)
            ...
            title_str =[' AP:'.join([class_list[idx], "{0:.2f}%".format(ap * 100)])+'   ' for idx, ap in enumerate(ap_list)]
            title_str.insert(0, 'Precision x Recall curve \n\n')
            title_str.append('\n mAP: %s' % mAP_str)
            # plt.title('Precision x Recall curve \nClass: %s, AP: %s' % (str(classId), ap_str))
            plt.title(' '.join(title_str))
            plt.savefig(os.path.join(savePath, 'all' + '.png'))
            plt.show()
        return results
    ...
    # For each pr_detections, calculate IOU with reference
    @staticmethod
